...
def write_seo_static_files(df):
    ...
    parts = parts[parts.ne("")].drop_duplicates().tolist()
    urls.extend(part_page_url(part) for part in parts)
...
def render_part_detail(inventory_df):
    requested_part = query_value("part", "").strip()
    ...
    if matched.empty:
        ...
        if st.button("Submit Manual RFQ", type="primary"):
    ...
    with right:
        ...
        whatsapp_text = quote_plus(
            f"Hello Mossab, I would like availability and pricing for part number {part_number}."
        )
        st.link_button(
            "WhatsApp Inquiry",
            f"https://wa.me/{WHATSAPP_NUMBER}?text={whatsapp_text}",
            use_container_width=True,
        )
        st.markdown(
            f"[Email Sales](mailto:{CONTACT_EMAIL}?subject=RFQ%20for%20{quote_plus(part_number)})"
        )
...
if page == "Home":
    ...
    display_result = result[SAFE_COLUMNS].copy()
    display_result["View Part"] = display_result["Part Number"].apply(part_page_url)
    st.dataframe(
        display_result,
        use_container_width=True,
        hide_index=True,
    ...
    with st.form("rfq_form"):
        ...
        for i in range(st.session_state.rfq_item_count):
            ...
            with cols[0]:
                default_part = query_value("part", "") if i == 0 else ""
